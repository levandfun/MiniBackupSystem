...
{
    Task<ManifestResponse?> SendManifestAsync(BackupJobConfig config, int sessionId, IEnumerable<FileMetadata> files, CancellationToken token);
    ...
    Task<int> CreateSessionAsync(BackupJobConfig config, CancellationToken token);
}